...
#Transfer files in the file directory
  ...
  do
    ...
    do
    	...
    	then	
    		(time dd bs=1M skip=$offset count=$block if=/$FILE_DIR/$file | md5sum)  &>> $cksum_output &
    ...
    if [ $filesize -ge $block ]
    then	
    	(time dd bs=1M skip=$offset_next count=$lastblock if=/$FILE_DIR/$file | md5sum)  &>> $cksum_output &
    else
	(time dd bs=1M skip=0 count=$lastblock if=/$FILE_DIR/$file | md5sum)  &>> $cksum_output &
    fi
